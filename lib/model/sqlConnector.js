...
var spQueries = {
	SPS: "select specific_schema as [schema], specific_name as name from information_schema.routines where routine_type = 'PROCEDURE' and specific_name like '%SP_NAME%'",
...
function SqlConnector(){
	...
	function sendSpResult(spName, params,values,callback){
		...
		if(params.length > values.length){
			callback({Error:"Service require " + params.length + " parameter/s. Only " + values.length + " parameter/s were provided"});
		}
	...
	function exposeSpAsWebService(serviceName, spInfo, callback){
		...
		sendQueryResult(query, function(spParamsInfo){
			...
			Services.insert({
				...
				if(result){
					callback({ Result: "successfully added a new service as " + serviceName});
				}
				else{
